...
contract VaultTest is Test {
    ...
    function testFuzzDeposits() public {
        ...
        address receiver = makeAddr("DEPOSIT_RECEIVER");
        uint256 amount = 100 * 10 ** 18;

        // Bound the amount to something reasonable (1 wei to 10 million tokens)
        ...
        if (amount > token.balanceOf(TOKEN_DEPLOYER)) {
            vm.prank(address(token));
            token.mint(TOKEN_DEPLOYER, amount);
        }
        ...
        assertEq(
            actualShares,
            expectedShares,
            "Shares received should match previewed amount"
        );
        assertEq(
            token.balanceOf(TOKEN_DEPLOYER),
        ...
        assertEq(
            ...
            initialShareBalance + actualShares,
            "Shares should be added to receiver"
        );
        assertEq(
            vault.totalAssets(),
            initialTotalAssets + amount,
            "Total assets should increase by deposit amount"
        );

        // Additional check: deposit twice and verify consistency
        token.approve(address(vault), amount);
        uint256 secondShares = vault.deposit(amount, receiver);

        // Verify shares scale consistently
        assertEq(
            secondShares,
            expectedShares,
            "Second deposit should yield same shares as first for same amount"
        );

        // Test conversion methods
        uint256 assetsForShares = vault.convertToAssets(actualShares);
        assertApproxEqRel(
            assetsForShares,
            amount,
            0.00001e18,
            "Asset/share conversion should be accurate"
        );

        // Verify maximum deposit functions correctly
        uint256 maxDeposit = vault.maxDeposit(receiver);
        assertTrue(
            maxDeposit >= amount,
            "Max deposit should be at least the amount we've already deposited"
        );

        // Test smaller subsequent deposit
        uint256 smallAmount = amount / 10 + 1;
        if (smallAmount > 0) {
            token.approve(address(vault), smallAmount);
            vault.deposit(smallAmount, receiver);
            assertEq(
                vault.totalAssets(),
                initialTotalAssets + amount + amount + smallAmount,
                "Total assets incorrect after multiple deposits"
            );
        }
    }
